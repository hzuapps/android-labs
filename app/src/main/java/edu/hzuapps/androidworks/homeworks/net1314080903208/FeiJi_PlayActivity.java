...
public class FeiJi_PlayActivity extends CCColorLayer {

	private List<FeiJi_SpriteActivity> _Foes = new CopyOnWriteArrayList<FeiJi_SpriteActivity>();
	private List<FeiJi_SpriteActivity> _BigFoes = new CopyOnWriteArrayList<FeiJi_SpriteActivity>();
	...
	private List<FeiJi_SpriteActivity> _AllFoes = new CopyOnWriteArrayList<FeiJi_SpriteActivity>();
	...
	private float _Shot_Du = 0.5f;// �ӵ��ٶ�
	private boolean _Can_Move = false;// �Ƿ��ڶ������ƶ�
	private CGSize _WinSize;
	private int _Play_Image_Chage = 1;// �ɻ�ͼƬ�ж�
	private int _BigFoe_Image_Chage = 1;// ���͵л�ͼƬ�ж�
	...
	private int _Big_Life = 16, _Middle_Life = 8, _Small_life = 2;// �л�����
	private int _ChangeImage_Delay = -1;// ͼƬ�ı���ʱ
	private int _Pause_OR = -1;// ��ͣ����ж�
	private int Red_Bomb_Num = 0;// ��ը������
	private boolean Blue_Shot_Change = false;// �Ƿ����ӵ�
	private long Blue_Shot_Last_Time = 0;// ���ӵ�����ʱ��
	private int Blue_Red_Down_Time = 30;// ���ӵ��ͺ�ը���������
	...
	protected FeiJi_PlayActivity(ccColor4B color) {
		super(color);
		Init();
	}
	...
	private void Init() {
		...
		_WinSize = CCDirector.sharedDirector().displaySize();// ��ȡ��Ļ��С
		...
		_FeiJi_Back.setPosition(CGPoint.make(_WinSize.width / 2,
				_WinSize.height / 2));// Ĭ�����ĵ�Ϊ���½ǣ����������ĵ��Ƶ���Ļ�м�
		addChild(_FeiJi_Back);// ��������ӵ�����
		...
		addChild(_FeiJi_Pause);// �����ͣ

		AddScore();

		this.schedule("GameFoes", 0.5f);// һ��ִ��һ��
	...
	private void AddFoes() {
		...
		FeiJi_SpriteActivity _FeiJi_Foe = new FeiJi_SpriteActivity();
	...
	public void AddBigFoe(float t) {
		if (_BigFoes.size() > 0) {
			...
			FeiJi_SpriteActivity BigFoe = (FeiJi_SpriteActivity) _BigFoes.get(0);
			if (_Play_Image_Chage == 1)
	 ...
	 * ��ӵл�����
	 */
	private void FoeDown(FeiJi_SpriteActivity _FeiJi_Foe2, int i) {
		Random rand = new Random();
	 ...
	 * ����ʱ���λ�ã���ӵл�����
	 */
	private void Down(FeiJi_SpriteActivity _FeiJi_Foe, int i, float y) {
	...
	public void Foe_Over(Object sender) {
		...
		for (int i = 0; i < _Foes.size(); i++) {
			FeiJi_SpriteActivity foe = _Foes.get(i);
	...
	public void BigFoe_Over(Object sender) {
		...
		for (int i = 0; i < _BigFoes.size(); i++) {
			FeiJi_SpriteActivity foe = _BigFoes.get(i);
	...
	private void PlayOver() {
		...
		for (int j = 0; j < _BigFoes.size(); j++) {
			FeiJi_SpriteActivity BigFoe = (FeiJi_SpriteActivity) _BigFoes.get(j);
		...
		for (int j = 0; j < _Foes.size(); j++) {
			FeiJi_SpriteActivity Foe = _Foes.get(j);
	...
	private void ReMoveAll() {
		List<FeiJi_SpriteActivity> _FoesAll = _Foes;
		List<FeiJi_SpriteActivity> _BigFoesAll = _BigFoes;
		for (int j = 0; j < _FoesAll.size(); j++) {
			FeiJi_SpriteActivity Foe = _FoesAll.get(j);
			Foe.Life = 0;
			if (Foe.Life <= 0) {
				if (Foe.getCCSprite().getTag() == 2) {
					ChageScore(1000);
		...
		for (int j = 0; j < _BigFoesAll.size(); j++) {
			FeiJi_SpriteActivity BigFoe = (FeiJi_SpriteActivity) _BigFoesAll.get(j);
			BigFoe.Life = 0;
			if (BigFoe.Life <= 0) {
				BigFoe.getCCSprite().removeSelf();
				ChageScore(30000);
	 ...
	 * @param i
	 */
	private void ChageSpriteBack(FeiJi_SpriteActivity Foe, boolean Click, String Path,
								 int i) {
		FeiJi_SpriteActivity _FeiJi_Foe = new FeiJi_SpriteActivity();
	...
	private void IntentToBack() {
		Intent mainScore = new Intent(
				CCDirector.sharedDirector().getActivity(), FeiJi_MenuActivity.class);
